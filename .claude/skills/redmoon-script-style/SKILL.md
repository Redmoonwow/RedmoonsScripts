...
## 12. なぜこの作法なのか — クリティカル系の作法が土台

この文書の規則はほぼすべて、**本番でデバッグできない前提**から出ている。
作者の本業がクリティカル案件（動的確保禁止・境界の明示・フェイルセーフ・静的解析必須の世界）で、
その作法がそのまま持ち込まれている。**好みではなく職業訓練**なので、
理由が分からないまま「今風」に直すと目的を壊す。

各規則を、元の分野の名前に対応させるとこうなる。

| この文書での呼び方 | 元の分野での名前 |
|---|---|
| 8 人分解いてから `Count() != 8` で弾く (§4) | **妥当性検査 (plausibility check)** |
| 合わなければ `ExceptionReturn` して描かない (§4, §11) | **フェイルセーフ状態への遷移** |
| `_partyDataList` に状態を集約 (§4) | **単一の真実の源 (single point of truth)** |
| 中核で例外を捕まえて恒久停止 (§11) | **ラッチ付きフォールト処理 / ウォッチドッグ** |
| 目で検証できる大きさに保つ (§14) | **レビュー可能性 (reviewability)** |
| 固定長テーブルを起動時に確保 | **静的メモリ割り当て** |

`Splatoon/Splatoon/Memory/BuffEffectProcessor.cs` がこの作法の見本。
初期化時に全メモリを確保して運用中は確保しない、上限に根拠のコメントを付ける
(`MAX_OBJECT_NUM = 299` に「629 スロットあるが EventObject までで足りる」)、
故障したら再試行せずラッチで止める。

Splatoon スクリプトという環境がこの作法と相性がよい理由もはっきりしている。
**1 ファイル完結・共有ライブラリ無し・実行時コンパイル・デバッガ実質不可・検証手段は録画とログのみ。**
単体テストで担保できないので、レビュー可能性の価値が相対的に高い。

### ただし、癖の失敗モードは自分の目に見えない

`BuffEffectProcessor.Dispose` には null 参照の不具合があるが、**null 検査自体は 2 つとも書かれている**。
外側ポインタの検査が、それを `[i]` で参照し終わった後に置かれているだけ。

```csharp
for(var i = 0; i < MAX_OBJECT_NUM; ++i)
    if(_CharacterStatusInfoPtr[i].StatusPtr != null) { ... }   // 内側は検査している
if(_CharacterStatusInfoPtr != null) { ... }                    // 外側も検査している。使った後に
```

目視レビューは「検査があるか」をパターンとして探すので、**あれば通過してしまう**。
欠陥は順序にあり、それを見るには制御フローを追う必要がある。
つまり **癖がある人の抜けほどレビューで捕まりにくい**。
`.editorconfig` とアナライザは、癖の代わりではなく **癖の失敗モードを補うため**に入れる。
