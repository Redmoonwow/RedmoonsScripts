...
public unsafe class P3_Earthquake4vi : SplatoonScript<P3_Earthquake4vi.Config>
{
        ...
        uint p6, uint p7, uint p8, ulong targetId, byte replaying)
    {
        ...
        // A marker-derived slot is latched by ResolveSelfSlot and never revisited, so a slot
        // resolved before the party's markers had landed stays wrong for the rest of the
        // mechanic. That happens whenever a marker from an earlier window is still on the
        // player when the target debuff arrives: TryMarkerSlot reads the stale one and wins the
        // race against the real assignment. Any overhead marker moving is the signal that the
        // answer may have changed, so drop the marker-derived resolution and let ResolveSelfSlot
        // run again on the next frame with current data.
    ...
    // Drops only the slot and how it was decided, so ResolveSelfSlot picks it up again on the
    // next frame. Unlike ClearSelfResolution this leaves _selfCompletedWindow and
    // _selfBlackHoleTask alone: clearing those mid-window would replay guidance for a window
    ...
    private void AdvanceWindow(int sourceBucket)
    {
        ...
        Log($"hit {DirectionName(sourceBucket)} {_hitSources.Count}/{ExpectedSourcesByWindow[_currentWindow]} " +
            $"expected={DirectionName(expected)} self={DirectionName(_selfTetherBucket)} slot={_selfSlot} " +
    ...
    private void SetFinalLanding(FinalStackRole stackRole)
    {
        ...
        if (TryGetOwnRolePosition(out var role))
        {
            ...
            Log($"tower target role={role} anchor={Deg(FinalPairAnchorAngle()):F1} " +
                $"observed={_finalTowerPositions.Count} dest=({destination.X:F1},{destination.Z:F1})");
