...
internal unsafe class ScriptEventLogger :SplatoonScript
{
    ...
    public override void OnObjectEffect(uint target, uint entityId, uint actionId)
    {
        ...
        PluginLog.Information($"OnObjectEffect: {targetObj.Name}{targetObj.Position}(GID: {targetObj.GameObjectId} DID: {targetObj.DataId}) - {entityId} - {actionId}");
    }
    ...
    public override void OnVFXSpawn(uint target, string vfxPath)
    {
        ...
        if (target.GetObject() is IBattleNpc npc && (npc.BattleNpcKind == BattleNpcSubKind.Pet ||
            npc.BattleNpcKind == BattleNpcSubKind.None || npc.BattleNpcKind == BattleNpcSubKind.RaceChocobo)) return;
            ...
            _ = new TickScheduler(() =>
            {
                ...
                else
                {
                    PluginLog.Information(
                        ...
                        $"(GID: 0x{gameObject.EntityId.ToString("X8")} DID: {gameObject.DataId.ToString("X8")})");
                }
            }
            );
        }
    ...
    public override void OnActorControl(uint sourceId, uint command, uint p1, uint p2, uint p3, uint p4, uint p5, uint p6, uint p7, uint p8, ulong targetId, byte replaying)
    {
        ...
        PluginLog.Information($"OnActorControl: {sourceId} - {command} - {p1} - {p2} - {p3} - {p4} - {p5} - {p6} - {p7} - {p8} - {targetId} - {replaying}");
    }
    ...
    public override void OnActionEffectEvent(ActionEffectSet set)
    {
        ...
        if (set.Target == null)
            PluginLog.Information(
                ...
                $"(GID: 0x{set.Source.GameObjectId.ToString("X8")} DID: {set.Source.DataId.ToString("X8")})");
        else
            PluginLog.Information(
                ...
                $"(GID: 0x{set.Target.GameObjectId.ToString("X8")} DID: {set.Target.DataId.ToString("X8")})");
    }
